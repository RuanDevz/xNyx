...
const AdminPanel: React.FC = () => {
  ...
  const fetchLinks = async () => {
    try {
      ...
      const response = await axios.get<LinkItem[]>(
        `https://x-nyx-backend.vercel.app${endpoint}`
      );
  ...
  const handleAddLink = async () => {
    ...
    try {
      const endpoint = activeTab === "free" ? "/freecontent" : "/vipcontent";
      await axios.post(`https://x-nyx-backend.vercel.app${endpoint}`, newLink);
  ...
  const handleUpdateLink = async () => {
    ...
    try {
      const endpoint = activeTab === "free" ? "/freecontent" : "/vipcontent";
      await axios.put(
        `https://x-nyx-backend.vercel.app${endpoint}/${isEditing}`,
        newLink
      );
      setIsEditing(null);
      setNewLink({ name: "", link: "", category: "", createdAt: "" });
  ...
  const handleDeleteLink = async (id: number) => {
    ...
    try {
      const endpoint = activeTab === "free" ? "/freecontent" : "/vipcontent";
      await axios.delete(`https://x-nyx-backend.vercel.app${endpoint}/${id}`);
      fetchLinks();
    } catch (error) {
